 ...
 * @date 12/31/14
 */
public class RequestStatHour extends Configured implements Tool {
    ...
    public static class MapMaper extends Mapper<Object, Text, Text, Text> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            Map<String, Object> result =
                    parser.mapParser(key, value, context, Constants.ANALYSIS.LOG_STAT_HOUR);
            if (result != null) {
    ...
    public static class Reduce extends
        ...
        public void reduce(Text key, Iterable<Text> values,
            ...
            Map<String, Object> result = parser.reduceParser(key, values, Constants.ANALYSIS.LOG_STAT_HOUR);
            String rowKey = key.toString();
            AppPv appPv = (AppPv) result.get("appPv");
            String day = rowKey.substring(0, rowKey.length() - 2);
            String hour = rowKey.substring(rowKey.length() - 2);
            //pv入库
            Put pvPut =
                    HbaseUtil.getPut(day, hour, appPv.getRequestUrl(), JsonParser.toString(appPv));
