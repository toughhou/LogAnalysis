 ...
 * @date 12/31/14
 */
public class LogKpiDay extends Configured implements Tool {
    public static Logger LOG = Logger.getLogger(LogKpiDay.class);
    ...
    public static class MapMapper extends Mapper<Object, Text, Text, Text> {
        ...
        public void map(Object key, Text value, Context context)
            ...
            Map<String, Object> result =
                    logParser.mapParser(key, value, context, Constants.ANALYSIS.LOG_STAT_DAY);
            if (result != null) {
    ...
    public static class Reduce extends
        ...
        public void reduce(Text key, Iterable<Text> values,
                           Context context)
                throws IOException, InterruptedException {
            Map<String,Object>  result= logParser.reduceParser(key, values,Constants.ANALYSIS.LOG_STAT_DAY);
            String rowKey=key.toString();
            //pv入库
            Put pvPut =
                    HbaseUtil.getPut(rowKey, Constants.HBASE.LOG_FAMILY_All, "pv", CommonUtil.objtoStr(result.get("pv")));
            context.write(new ImmutableBytesWritable(key.getBytes()), pvPut);
            //uv入库
            Put uvPut =
                    HbaseUtil.getPut(rowKey, Constants.HBASE.LOG_FAMILY_All, "uv", CommonUtil.objtoStr(result.get("uv")));
            context.write(new ImmutableBytesWritable(key.getBytes()), uvPut);
            //uf入库
            Put ufPut =
                    HbaseUtil.getPut(rowKey, Constants.HBASE.LOG_FAMILY_All, "uf", CommonUtil.objtoStr(result.get("uf")));
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setJarByClass(LogKpiDay.class);
...
//        String currentDate = DateUtil.getBeforeDays(1, "yyyyMMdd");
        ...
        ToolRunner.run(new Configuration(), new LogKpiDay(), args1);
    }
}
